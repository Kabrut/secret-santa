...
function AdminPanel({ config, onUpdate, apiUrl }) {
  ...
  const fetchDraws = async () => {
    ...
    try {
      const response = await axios.get(`${apiUrl}/api/draws`);
    ...
    <div className="bg-white rounded-2xl shadow-2xl p-8">
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {loadingDraws ? 'Ładowanie...' : '🔄 Odśwież'}
          </button>
        </div>

        {draws.length === 0 ? (
          <div className="p-8 bg-gray-50 rounded-lg text-center">
            <p className="text-gray-600">Brak losowań. Użytkownicy jeszcze nie rozpoczęli losowania.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full border-collapse bg-white rounded-lg overflow-hidden shadow">
              <thead className="bg-christmas-green text-white">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left font-semibold">Kto losował</th>
                  <th className="px-6 py-3 text-center font-semibold">→</th>
                  <th className="px-6 py-3 text-left font-semibold">Wylosowana osoba</th>
                  <th className="px-6 py-3 text-left font-semibold">Data losowania</th>
                </tr>
              </thead>
              <tbody>
                {draws.map((draw, index) => (
                  <tr key={index} className="border-b border-gray-200 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 font-semibold text-gray-800">{draw.giver}</td>
                    <td className="px-6 py-4 text-center text-2xl">🎁</td>
                    <td className="px-6 py-4 font-semibold text-christmas-red">{draw.receiver}</td>
                    <td className="px-6 py-4 text-gray-600 text-sm">
                      {new Date(draw.drawnAt).toLocaleString('pl-PL')}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>📊 Podsumowanie:</strong> {draws.length} {draws.length === 1 ? 'osoba wylosowała' : 'osób wylosowało'} 
                {config?.participants ? ` z ${config.participants.length} uczestników` : ''}
              </p>
            </div>
          </div>
        )}
